 ...
 *
 ...
 it has been changed to in clude the 1 second delay
 */
...
const int btnPin = 13;  //button is attached to pin 13, and is wired up based on example 5 from arduino uno kit
...
void setup() {
  //ESP.wdtDisable(); //supposedlydisables the watch dog timer
...
void loop() {
  ...
  if(btnState == LOW) //if button is pushed
  {
    if(wasPressed == false)//and it wasn't pushed before
    {
      ...
      time1 = millis();
      if(time1-time2 > 250)////iwant it to be a quarter second. it has been a second! i tihnk is what this means
      {
