...
void setup() {
  //ESP.wdtDisable(); //supposedly disables the watch dog timer
...
void loop() {
  ...
  if(btnState == LOW) //if button is pushed
  {
    if(wasPressed == false)//and it wasn't pushed before
    {
      ...
      if(time1-time2 > 250)
      {
        wasPressed = true; //say that it has just been pressed
    ...
    else if(wasPressed == true)//the button was down/pushed before now.
    {
      return;
    }
  }
